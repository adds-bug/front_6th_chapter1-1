...
`,a=Array.from({length:4},()=>i).join(``),o=e=>`
  ...
  `,c=({products:e=[],loading:t=!1,total:n=0,categories:r,limit:i=20,sort:c=`price_asc`})=>{let l=Object.keys(r);return`
          ...
          <div class="space-y-3">
            ...
            <div class="space-y-2">
              ...
              <div class="flex flex-wrap gap-2">
                ${t?`<div class="text-sm text-gray-500 italic"> 카테고리 로딩 중... </div>`:`${l.map(s).join(``)}`}
              </div>
              <!-- 2depth 카테고리 -->
            ...
            <div class="flex gap-2 items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                ...
                <select id="limit-select" class="text-sm border border-gray-300 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                  <option value="10" ${i===10?`selected`:``}>10개</option>
                  <option value="20" ${i===20?`selected`:``}>20개</option>
                  <option value="50" ${i===50?`selected`:``}>50개</option>
                  <option value="100" ${i===100?`selected`:``}>100개</option>
                </select>
              </div>
              <!-- 정렬 -->
              <div class="flex items-center gap-2">
                ...
                <select id="sort-select" class="text-sm border border-gray-300 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                  <option value="price_asc" ${c===`price_asc`?`selected`:``}>가격 낮은순</option>
                  <option value="price_desc" ${c===`price_desc`?`selected`:``}>가격 높은순</option>
                  <option value="name_asc" ${c===`name_asc`?`selected`:``}>이름순</option>
                  <option value="name_desc" ${c===`name_desc`?`selected`:``}>이름 역순</option>
                </select>
              </div>
            </div>
          </div>
  ...
  `};async function l(e={}){let{limit:t=20,search:n=``,category1:r=``,category2:i=``,sort:a=`price_asc`}=e,o=e.current??e.page??1,s=new URLSearchParams({page:o.toString(),limit:t.toString(),...n&&{search:n},...r&&{category1:r},...i&&{category2:i},sort:a}),c=await fetch(`/api/products?${s}`);return await c.json()}async function u(){let e=await fetch(`/api/categories`);return await e.json()}const d=()=>r(async()=>{let{worker:e}=await import(`./browser-CfvsmY1_.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`,serviceWorker:{url:`/front_6th_chapter1-1/mockServiceWorker.js`}})),f={products:[],total:0,loading:!1,categories:{}};function p(){let e=new URLSearchParams(location.search);return{limit:parseInt(e.get(`limit`))||20,sort:e.get(`sort`)||`price_asc`}}function m(e){let t=new URLSearchParams(location.search);Object.entries(e).forEach(([e,n])=>{t.set(e,n)}),history.pushState(null,``,`?${t.toString()}`)}function h(){document.body.addEventListener(`change`,async e=>{if(e.target.id===`limit-select`||e.target.id===`sort-select`){let t=new URLSearchParams(location.search),n=parseInt(t.get(`limit`))||20,r=t.get(`sort`)||`price_asc`,i=e.target.id===`limit-select`?parseInt(e.target.value):n,a=e.target.id===`sort-select`?e.target.value:r;m({limit:i,sort:a}),f.loading=!0,g();let[{products:o,pagination:{total:s}}]=await Promise.all([l({limit:i,sort:a})]);f.products=o,f.total=s,f.loading=!1,g()}})}function g(){let{limit:e,sort:t}=p();document.body.querySelector(`#root`).innerHTML=c({...f,limit:e,sort:t})}async function _(){f.loading=!0,g();let{limit:e,sort:t}=p(),[{products:n,pagination:{total:r}},i]=await Promise.all([l({limit:e,sort:t}),u()]);f.products=n,f.total=r,f.categories=i,f.loading=!1,g(),h()}d().then(_);